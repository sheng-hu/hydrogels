        ...
        for line in file:
            ...
            if match:
                ...
                if first_number >= threshold:
                    # print(first_number)
                    key = (fractions, ml_predicted, first_number)
                    fraction_counts[key] += 1  # Count occurrences

    # Convert dictionary to a sortable list
    for (fractions, ml_predicted, first_number), count in fraction_counts.items():
        fraction_data.append((fractions, ml_predicted, first_number, count))

    # Sort the results by ML predicted value (descending order)
    fraction_data.sort(key=lambda x: x[2], reverse=False)

    count_stop = 0
    print("Identical Fractions & ML Predictions Count (Sorted by EI value):")
    for fractions, ml_predicted, first_number, count in fraction_data:
        if count_stop > show_num:
            break
        if count > cutoff:
            print(
                f"{first_number}: {fractions} ML predicted: {ml_predicted:.6f} → Count: {count}"
            )
            if count > 100:
                count_stop += 1
